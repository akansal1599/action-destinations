import { ActionDefinition, RequestClient, FieldValidationError } from '@segment/actions-core'
...
function validate(payloads: Payload[]): void {
  if (payloads[0].cohort_name !== payloads[0].personas_audience_key) {
    throw new FieldValidationError('The value of `personas computation key` and `personas_audience_key` must match.')
  }
}
