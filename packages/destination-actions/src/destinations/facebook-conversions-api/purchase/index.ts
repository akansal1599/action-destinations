import { ActionDefinition, FieldValidationError } from '@segment/actions-core'
...
const action: ActionDefinition<Settings, Payload> = {
  ...
  perform: (request, { payload, settings, features, statsContext }) => {
    if (!CURRENCY_ISO_CODES.has(payload.currency)) {
      throw new FieldValidationError(`${payload.currency} is not a valid currency code.`)
    }

    if (!payload.user_data) {
      throw new FieldValidationError('Must include at least one user data property')
    }

    if (payload.action_source === 'website' && payload.user_data.client_user_agent === undefined) {
      throw new FieldValidationError('If action source is "Website" then client_user_agent must be defined')
    }
